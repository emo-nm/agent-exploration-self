...
export class EveAdapter {
    ...
    async *streamMessage(
        ...
        const persist = async () => {
            ...
            const cursor: SessionState = {
                ...
                continuationToken:
                    base.continuationToken ?? response.continuationToken,
            } as SessionState;
            await this.threads.saveContinuation(appThreadId, {
    ...
    async *streamEvents(appThreadId: string): AsyncIterable<AgentEvent> {
        ...
        if (!saved?.sessionId) {
            throw new Error(
                `thread ${appThreadId} has no eve session to stream`,
            );
        }
    ...
    switch (e.type) {
        ...
        case "action.result": {
            ...
            return {
                type: "tool-result",
                toolName: String(
                    result.toolName ?? result.subagentName ?? "unknown",
                ),

...
export function searchFixtureCorpusImpl(input: { query: string; maxResults?: number }) {
...
export const makeCreatePublicationProposalTool = (repo: DemoRepo) =>
  createTool({
    ...
    execute: async (inputData) => {
      const proposal = await createPublicationProposalImpl(repo, inputData);
...
export const makeGetPublicationStatusTool = (repo: DemoRepo) =>
  createTool({
    ...
    execute: async (inputData) => getPublicationStatusImpl(repo, inputData.proposalId),
  });
